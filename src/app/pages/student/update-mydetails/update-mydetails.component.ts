...
@Component({
  selector: 'app-update-mydetails',
  templateUrl: './update-mydetails.component.html',
  styleUrls: ['./update-mydetails.component.css']
})
export class UpdateMydetailsComponent implements OnInit {
  ...
  constructor(private fb:FormBuilder,private studentserv:StudentService,private router:Router,
    ...
    private SpinnerService: NgxSpinnerService) {
      ...
      this.studentserv.getdata('editstudent/'+this.logininfo['user_id']).then(async res => {
...
//             });
  ...
  gotopersonaldetailsform(stepper: MatStepper){
    ...
    // this.isShowwordmaxlengtherror = false;
    // if(this.isShowwordmaxlengtherror == false){
    //   stepper.next();
    // }
    // this.col_submitted = true;
    // this.fees_submitted = true;
    ...
    console.log(this.transfer_option);
      ...
      if(this.transfer_option == 'DD'){
        myFormData.append('transfer_option', 'DD');
      ...
      myFormData.append('dd_favouring', this.collegeDetailsFormGroup.value.dd_favouring);
        ...
        myFormData.append('transfer_option', 'RTGSORNEFT');
      ...
      // fees details
        ...
        myFormData.append('fees_type',JSON.stringify(this.addfeesForm.controls['feestype_array'].value));
          this.SpinnerService.show();
          this.studentserv.postmethod('updatestudentprofile',myFormData,true).then(res => {
            this.SpinnerService.hide();
            if(res['status'] == 'success'){
              Swal.fire({
                title: 'Updated',
    ...
    onDateChanged(event, text) {
      ...
      if (text == 'birth_date') {
        this.personalDetailsFormGroup.controls['dateofbirth'].setValue(event.singleDate.formatted);
        ...
        this.collegeDetailsFormGroup.controls['join_date'].setValue(event.singleDate.formatted);
        let start_date = event['singleDate'].date;
        console.log(start_date.day, start_date.month, start_date.year)
        // this.myDatePickerOptions = {
        //    dateRange: false,
  ...
  console.log(this.words);
    ...
    if (words.length >= 150 && valid_keys.indexOf(event.keyCode) == -1) {
        // e.preventDefault();
        // words.length = 10;
        // event.target.value = words.join(' ')
        return false;
