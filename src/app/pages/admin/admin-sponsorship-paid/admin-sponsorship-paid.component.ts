...
@Component({
  selector: 'app-admin-sponsorship-paid',
  templateUrl: './admin-sponsorship-paid.component.html',
  styleUrls: ['./admin-sponsorship-paid.component.css']
})
export class AdminSponsorshipPaidComponent implements OnInit {
  ...
  constructor(private fb:FormBuilder,private Activate: ActivatedRoute,private studentserv:StudentService,
    private toastr:ToastrService,private sponsorser:SponsorsService,private router:Router,private spinnerser:NgxSpinnerService) { 
      ...
      if(this.student_id){
        this.studentserv.getdata('getstudentfeesdata/' + res.id+'/'+this.logininfo['user_id']).then(async res => {
          ...
          if (res['status'] == 'success') {
            var data = res['data'];
            var bank_data = res['bankdata'];
            this.sponsor_balance = bank_data[0].bank_balance;
            this.feesdata = [];
  ...
  paysponsorship(){
    ...
    if(this.paysponsorshipform.invalid){
      ...
      if(this.paysponsorshipform.value.paid == 0){
        this.toastr.info('paid amount should be greater than zero', 'info', {
          progressBar:true
        });
        return;
      }
      if(this.sponsor_balance < this.paysponsorshipform.value.paid){
        this.toastr.info('your bank balance amount is less than paid amount', 'info', {
          progressBar:true
        });
        return;
      }
